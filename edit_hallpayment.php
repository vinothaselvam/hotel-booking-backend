...
{
    extract($_POST);
    $sql4 = "SELECT SUM(amount) as amt from tbl_hallpayment WHERE bookingid='".$_GET["id"]."'";
                                    $result4 = $conn->query($sql4);
                                    $row4 = $result4->fetch_assoc();
   $paid_amount = $row4['amt'];
  
   $sql = "INSERT INTO `tbl_hallpayment`(`bookingid`,`amount`,`date`) VALUES ('$bookingid','$amount','$date')";

 if ($conn->query($sql) === TRUE) {
      $_SESSION['success']=' Record Successfully Added';
    }    

    $q1="UPDATE `tbl_hall` SET `paid`='$paid_amount' WHERE `id`='".$_GET['id']."'";
    //$q2=$conn->query($q1);
...
<?php
$que="SELECT * FROM `tbl_hall` WHERE id='".$_GET["id"]."'";
...
$date = $result3->num_rows > 0 ? $row3['date'] : date('Y-m-d');
$totalamount = $row['totalamount'];
$taxamount = $row['taxamount'];
$paid =  $row4['amt'];
$paid_amount = $row4['amt'];
$remainamount = $row['totalamount']-$row4['amt'];
                                    ...
                                    <form class="form-valide"  method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group row">
                                         ...
                                         <div class="form-group row">
                                            ...
                                            <div class="col-lg-6">
                                                <input type="text" class="form-control" id="val-digits" name="taxamount" value="<?php echo $totalamount; ?>" placeholder="Payable Amount"  required="" onchange="calculate();" onkeyup="calculate();" readonly>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group row">
                                            <label class="col-lg-4 col-form-label" for="val-digits">Advance amount / Paying Amount<span class="text-danger">*</span></label>
                                            <div class="col-lg-6">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-6">
                                                <input type="date" class="form-control" id="val-digits" name="date" placeholder="date" required="">
                                            </div>
                                        </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="table-responsive m-t-40">
                                    ...
                                    $sql = "SELECT * FROM `tbl_hall` join `tbl_hallpayment` 
                                    On tbl_hall.id= tbl_hallpayment.bookingid
                                    WHERE tbl_hall.id = '".$_GET['id']."'";
                                    
                                     $result = $conn->query($sql);
                                 
                                  while ($row = $result->fetch_assoc()) {
                                    
                                     $sql2 = "SELECT * FROM `tbl_hallpayment` WHERE id='".$row['id']."'";
